"""
n x n 크기의 빈 격자와, 정수 jump가 주어집니다. 당신은 이 격자의 최 좌측 상단 칸부터 달팽이처럼 시계방향으로 회전하면서 1부터 n2 까지의 숫자를 
차례대로 채우려고 합니다.
...
격자의 세로 좌표는 위에서부터 아래로 1부터 n이며, 가로 좌표는 좌측에서 우측으로 1부터 n입니다. 따라서, 시작하는 칸은 1행 1열이 됩니다. 
격자의 크기를 나타내는 n과 숫자를 채워나가는 간격 jump가 매개변수로 주어집니다. 이때, 마지막 숫자가 들어가는 행과 열을 차례대로 배열에 담아서 
